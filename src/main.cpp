...
#if defined(NDEBUG)
# error "SkullCash cannot be compiled without assertions."
#endif
...
static const int64_t nTargetTimespan = 3 * 60;  // 5 mins
...
const string strMessageMagic = "SkullCash Signed Message:\n";
...
{
    int64_t nSubsidy = 1.855555 * COIN;
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified for SkullCash: softfork block set to min diff to re-engage chain movement
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //LogPrintf("SkullCash retargetted using: DGW-v3 difficulty algo \n");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal skullcash miner
...
#ifdef ENABLE_WALLET
// skullcash: attempt to generate suitable proof-of-stake
...
{
    RenameThread("skullcash-loadblk");
