...
#include "skullcashaddressvalidator.h"
...
{
    widget->setMaxLength(SkullCashAddressValidator::MaxAddressLength);
    widget->setValidator(new SkullCashAddressValidator(parent));
    widget->setFont(skullcashAddressFont());
}
...
bool parseSkullCashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("skullcash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SkullCashUnits::parse(SkullCashUnits::SKULL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSkullCashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert skullcash:// to skullcash:
    //
    //    Cannot handle this later, because skullcash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("skullcash://"))
    {
        uri.replace(0, 12, "skullcash:");
    }
    QUrl uriInstance(uri);
    return parseSkullCashURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open skullcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SkullCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SkullCash.lnk
...
{
    return GetAutostartDir() / "skullcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a skullcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SkullCash\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SkullCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  skullcash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SkullCash-Qt"));
