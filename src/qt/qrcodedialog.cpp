...
{
    QString ret = QString("skullcash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SKULL unit input in lnReqAmount, we generate the URI with SKULL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SkullCashUnits::format(SkullCashUnits::SKULL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
